...
3. 运行 `csv_file_arrangement.py`，生成训练数据 `input.csv` 和 `output.csv`。
4. 运行 `PLSTM.py`，生成预测结果 `predictions.csv`。
5. 运行 `ADDTime.py`，为预测结果添加时间列。
6. 运行 `COMPARE2.py` （注意修改要对比的 CSV 文件名称）查看预测结果。
---
...
3. Run `csv_file_arrangement.py` to generate the training datasets `test.csv` and `output.csv`.
...
6. Run `COMPARE2.py` (remember to modify the CSV filenames you want to compare) to view the prediction results.
---
...
3. Führe `csv_file_arrangement.py` aus, um die Trainingsdaten `input.csv` und `output.csv` zu erstellen.
4. Führe `PLSTM.py` aus, um die Vorhersageergebnisse in `predictions.csv` zu generieren.
5. Führe `ADDTime.py` aus, um den Vorhersageergebnissen eine Zeitspalte hinzuzufügen.
6. Führe `COMPARE2.py` aus (achte darauf, die CSV-Dateinamen anzupassen, die du vergleichen möchtest), um die Vorhersageergebnisse anzusehen.
---
